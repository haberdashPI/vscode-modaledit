...
export async function handleKey(key: string, selecting: boolean,
    capture: boolean, selectflag: boolean, userInitiated: boolean): Promise<boolean> {
    ...
    if (capture && lastCommand)
        ...
        if (currentKeymap[key])
            await execute(currentKeymap[key], selecting, selectflag)
        else
            error()
    }
    else {
        if (selecting && selectKeymap[key])
            await execute(selectKeymap[key], selecting, selectflag)
        else if (baseKeymap[key])
            await execute(baseKeymap[key], selecting, selectflag)
        else
            error()
    }
